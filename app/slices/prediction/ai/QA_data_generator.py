...
# MODIFICATION HERE: Add api_key as an argument
def process_frame(frame_num, observation, input_prompt, cache_path, api_key): # <--- ADD api_key
    # MODIFICATION HERE: Set the API key within the process
    openai.api_key = api_key # <--- ADD THIS LINE
    ...
    response_content = make_description_from_prompt(input_prompt) # This will now use the key set above
    result = {
        "frame_num": frame_num,
...
def main(
    ...
    cached_labels_filename = os.path.join(output_folder, "training_QA_data.pkl")
