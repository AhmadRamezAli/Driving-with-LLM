...
from .attention_core import Attention
...
def generate_square_subsequent_mask(
    ...
    # diagonal argument for tril:
    # if k=0, include diagonal.
    # if k>0, exclude k diagonals above main.
    # if k<0, exclude k diagonals below main.
    # The logic `diagonal + max(num_keys - num_queries, 0)` is to correctly handle
    # cases where num_keys (context length including past) is different from num_queries (current input length).
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: Tensor, context: Optional[Tensor] = None, mask: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        ...
        if self.pre_norm2 is not None: # extra_context_norm is True
            ...
            # This case implies context should be normed by a norm suitable for its dimension.
            # If context_dim == model_dim, pre_norm1 could be used.
            # Otherwise, context should ideally be pre-normed or an error raised.
            if context_to_use.shape[-1] == x.shape[-1]: # Context dim same as x dim
                normed_context = self.pre_norm1(context_to_use)
                  ...
                  # This was the problematic case in the original logic.
                  # The attention module itself might handle this (e.g. PlainAttention has to_kv from context_dim)
                  # Pass context_to_use as is, assuming attention module's projections handle it.
                normed_context = context_to_use
            y, attn = self.attention.forward(normed_x, context=normed_context, mask=mask)
        else: # Self-attention, extra_context_norm is False
            # For self-attention, K,V come from x. So use normed_x.
            # The 'context' arg to attention.forward will be normed_x.
