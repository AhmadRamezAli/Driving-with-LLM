...
class CrossPerceiver(nn.Module):
    ...
    def forward(self, x: Tensor, context: Tensor, context_mask: Optional[Tensor]):
        # x: [batch, model_dim] (query)
        # context: [batch, context_len, context_dim]
        # context_mask: [batch, context_len] (True means attend)
        ...
        # aggregated_attention_maps: stores sum of attention weights for each context token
        # Shape: (B, context_len)
