...
class StopAfterFirstCheckpointCallback(TrainerCallback):
    ...
    def on_step_begin(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        if self.initial_global_step_this_run == -1 and state.global_step > 0 : # state.global_step > 0 indicates actual training started
            ...
            # If resuming, state.global_step might already be > 0.
            # We want to stop after the *next* save.

    def on_save(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        """
        Event called after a checkpoint save.
        """
        # Check if this is the first save operation *of the current training session*
        # and that we have actually made progress beyond any resumed checkpoint.
        if not self.first_checkpoint_saved_this_run and \
           (self.initial_global_step_this_run != -1 and state.global_step >= self.initial_global_step_this_run):
            # The condition `state.global_step >= self.initial_global_step_this_run` ensures
            # we are looking at a save that happened *during or after* the first training step of this run.
            # A more robust check might be to compare against `state.global_step` at the start of `on_train_begin`
            # if the trainer immediately saves a resumed checkpoint without taking a step.
            # However, `on_save` is typically called *after* a step that triggers the save.

            # We also need to ensure that the save is happening because we hit a save_steps interval
            # and not just an initial save if load_best_model_at_end=True from a previous run
            # or an end-of-training save.
            # The most straightforward way is to assume any save after training has started progressing
            ...
            # More precise: check if this save is due to hitting a save_steps interval
            # or if it's the very first step if save_steps=1 (or very small)
...
class TrainerWithGeneration(transformers.Seq2SeqTrainer):
    ...
    def evaluation_loop(
        ...
        for batch in dataloader:
            # 'user_input_ids' is what you pass to generate, 'input_ids' is the full sequence for training loss
            # Let's assume you're using 'user_input_ids' for generation prompt. If not, use 'input_ids'.
            # Your `label_names` in TrainingArguments includes 'user_input_ids'.
            if 'user_input_ids' in batch:
                ...
                # Fallback if 'user_input_ids' is not present in the eval batch
                # This might happen if you use a different data collator for eval
...
def train(
    ...
    # wandb_project: str = "llm-driver-stage1",
    # wandb_run_name: str = "stage1",
...
,
    ...
    # Select a smaller, random subset for faster validation
    # For example, 100 samples instead of 1000
        num_eval_samples = 100 
        val_data = val_data.shuffle(seed=42).select(range(num_eval_samples))
    # print(f"INFO: Using a smaller validation set of {len(val_data)} samples for speed.")
# --- END: THE FIX ---
...
# Inside your train function
    ...
    training_args = transformers.Seq2SeqTrainingArguments( # Assign to a variable first
        ...
        metric_for_best_model="loss", # Or another metric like "accuracy", "rouge" etc.
                                      # depending on what your evaluation_loop computes and returns.
                                      # For Seq2Seq, often a generation metric like ROUGE or BLEU,
                                      # or simply 'eval_loss' if predict_with_generate=False during eval.
                                      # If predict_with_generate=True, eval_loss might not be the primary metric.
    ...
    if mode == "train":
        ...
        # Pass the determined path or a boolean to trainer.train()
        # If determined_resume_path is None, passing None or False means start fresh.
        # If determined_resume_path is a path string, Trainer will use it.
        # If you want Trainer to auto-detect latest in output_dir, pass True, but your logic above does this.
